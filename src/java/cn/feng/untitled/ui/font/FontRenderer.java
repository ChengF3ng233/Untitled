...
public class FontRenderer extends Font {
    ...
    private float getCharWidthFloat(char c) {
        if (c == 167) {
            ...
            if (c > 0 && var2 != -1) {
                return this.defaultCharacters[var2].width() / 2.0F - 4.0F;
            } else if (c < this.defaultCharacters.length && this.defaultCharacters[c].width() / 2.0F - 4.0F != 0.0F) {
                int var3 = (int) (this.defaultCharacters[c].width() / 2.0F - 4.0F) >>> 4;
                int var4 = (int) (this.defaultCharacters[c].width() / 2.0F - 4.0F) & 15;
                var3 &= 15;
                ++var4;
            ...
            for (int i = 0; i < length; ++i) {
                ...
                if (previousCharacter != 167) {
                    if (character == 167) {
                        ...
                        width = (int) ((float) width + (characterSet[character].width() - 8.0F));
                    }
                }
