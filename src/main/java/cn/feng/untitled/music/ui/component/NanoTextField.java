...
public class NanoTextField {
    ...
    public NanoTextField(float width, float height, NanoFontRenderer font, Color backgroundColor, Color outlineColor) {
        this.text = "";
        this.width = width;
    ...
    public void draw(float x, float y, int mouseX, int mouseY) {
        ...
        NanoUtil.drawRoundedOutlineRect(posX, posY, width, height, radius, 1f, backgroundColor, outlineColor);
        ...
        font.drawString(visibleText, textX + offsetX + 1f, textY, size, NanoVG.NVG_ALIGN_MIDDLE, textColorAnim.getOutput(), true);

        if (focused) {
            float h = height * 0.6f;
            NanoUtil.drawRect(textX + offsetX + font.getStringWidth(visibleText) + 1.5f, posY + height / 2 - h / 2, 0.5f, h, cursorColorAnim.getOutput());
        }
    }
